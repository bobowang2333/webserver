...
int http_session(int *connect_fd,struct sockaddr_in *client_addr)
{
	...
	unsigned char send_buf[SEND_BUFFER_SIZE + 1 ];//server socket send buffer
	unsigned char file_buf[FILE_MAX_SIZE + 1];
	memset(recv_buf,'\0',sizeof(recv_buf));
	memset(send_buf,'\0',sizeof(send_buf));
	memset(file_buf,'\0',sizeof(file_buf));

	char uri_buf[URI_SIZE+1];   //store the uri request from client
    ...
    while(flag)
    {
    	res = select(maxfd,&read_set,NULL,NULL,&timeout);  //select?????
        switch(res)
        {
        	case -1 :
...
int is_http_protocol(char *msg_from_client)
{
	...
	if(strncmp(msg_from_client+index-10,"HTTP/",5) == 0)   //strncmp?????
		//HTTP request first line like this'GET /index.html HTTP/1.1',so last 10 bytes are HTTP/1.1\r\n???  last 10???
	{
		return 1;
	}
	return 0;
...
char *get_mime_type(char *uri)
{
	...
	switch(type_len)
	{
		...
		break;
		default:   //unkonwn type or server do not support type now
		return "NULL";
		break;
	}
	return NULL;
...
int reply_normal_information(unsigned  char * send_buf,unsigned char * file_buf,int file_size, char *mime_type)
{
	...
	register int index = strlen(str);    //register???
