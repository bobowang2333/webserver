...
int init_socket(int *listen_fd ,struct sockaddr_in *server_addr)
{
	...
	{
		perror("socket() error, in init_socket.c");//perror???
		return -1;
	}

	//set reuse the port on server machine
	int opt = SO_REUSEADDR;
	if(setsockopt(*listen_fd,SOL_SOCKET,SO_REUSEADDR,&opt,sizeof(opt)) ==  -1)//setsockopt?????
         {
         ...
         if(listen(*listen_fd,BACKLOG) == -1)    //listen ??
         {
